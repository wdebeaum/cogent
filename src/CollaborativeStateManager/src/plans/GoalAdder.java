...
public class GoalAdder {
	...
	public KQMLList getGoalAdditionType(KQMLList innerContent, KQMLList context, String id, String what)
	{
		...
		if (asObject != null && asObject instanceof KQMLList)
		{
			...
			case "ELABORATION":
				...
				gp.getGoalUnderDiscussion() == null)
				{
					return Messages.missingActiveGoal(what, ActType.PROPOSE, context, gp, or);
				}
			...
			case "ANSWER":
				...
				if (toObject != null)
				{
					if (!gp.hasGoal(toObject.stringValue()))
						return Messages.missingGoal(toObject.stringValue(), context);
				...
				{
					return Messages.missingActiveGoal(what, ActType.ANSWER, context, gp, or);
				}
				break;
		...
		else if (currentAcceptedGoal == null)
		{
			...
			if (!gp.hasAcceptedGoal && !or.isRootGoal(goalType))
				return Messages.missingActiveGoal(goalType,what, ActType.PROPOSE, context, gp, or);
		...
		else // Currently adds as subgoal by default
		{
			...
			else
			{
				...
				speechAct.add(what);
				responseContent = Messages.unknownGoalRelation(speechAct, 
											gp.getActiveGoal().getId(), newGoal.getId(), 
											context);
			}
	...
	{
		KQMLObject beneficiary = KQMLUtilities.getLinkedArguments(goalTerm, context, 
													":BENEFICIARY",":GROUND",":REFERS-TO");
		KQMLObject parentBeneficiary = KQMLUtilities.getLinkedArguments(
									gp.getActiveGoal().getKQMLTerm(), context,
									":BENEFICIARY",":GROUND",":REFERS-TO");
		...
		if (beneficiary != null && (parentBeneficiary == null || 
									!parentBeneficiary.equals(beneficiary)))
			return false;
